...
namespace Mercado_Vera.View.GerVenda.Relatorios
{
    public partial class FmrVendaDetalhada : Form
    {
        ...
        public void GetDivida(string divida)
        {
            lblToTalDivida.Text = divida;
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            for (Int32 i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if ( i <= 1)
                {
                    ...
                    linha = "-----------------------------------------------------";
                    //ITENS
                    string Desc = "DESC                                        QTD    VALOR  DATA";
                    //BOTTOM
                    ...
                    e.Graphics.DrawString(Desc, new Font("Arial", 8), new SolidBrush(Color.Black), new Point(3, 130));
                }
                ...
                DateTime formatDate = DateTime.Parse(data);
                data = formatDate.ToShortDateString();
                ...
                e.Graphics.DrawString(prodNome, new Font("Arial", 7), new SolidBrush(Color.Black), 3, offset);
                e.Graphics.DrawString(qtd, new Font("Arial", 7), new SolidBrush(Color.Black), 169, offset);
                e.Graphics.DrawString(total, new Font("Arial", 7), new SolidBrush(Color.Black), 200, offset);
                e.Graphics.DrawString(data, new Font("Arial", 7), new SolidBrush(Color.Black), 240, offset);
                offset += 20;

            }
            //bottom
            offset += 15;
            e.Graphics.DrawString("TOTAL:                                                 " + lblTotalSoma.Text, new Font("Arial", 8, FontStyle.Bold), new SolidBrush(Color.Black), 3, offset);
            offset += 10;
            ...
            e.Graphics.DrawString("Valor em aberto: " + lblToTalDivida.Text, new Font("Arial", 10), new SolidBrush(Color.Black), 3, offset);
            
            string dataAtual = DateTime.Now.ToShortDateString();
            e.Graphics.DrawString("Data: " + dataAtual, new Font("Arial", 10), new SolidBrush(Color.Black), 180, offset);
            offset += 20;
        }
    }
}
